...
def _post(job_id: str, close_date: datetime | None = None) -> JobPost:
    return JobPost(
        job_id=job_id,
        guild_id=1,
        forum_post_id=99,
        ...
        close_date=close_date or datetime.now(tz=timezone.utc) + timedelta(days=30),
    )
...
async def _run_fix_tags(
    ...
    with (
        patch(
            ...
            new=AsyncMock(return_value=[_post(job_id, close_date=close_date)]),
        ),
        patch(
        ...
        patch(
            ...
            new=AsyncMock(return_value=jobs if jobs is not None else {}),
        ),
        patch(
...
async def test_ineligible_job_thread_is_left_archived():
    # In the collection, but scored as not board material.
    thread = await _run_fix_tags(
        "job-1", eligible_ids=set(), jobs={"job-1": JobDocument(title="T")}
    )
...
async def test_a_still_open_job_missing_from_mongo_is_left_alone():
    # Not in the eligible set because it is not in the collection at all. A
    # source dropping a listing is not the role closing, so a deadline that has
    # not passed keeps the thread on the board rather than hiding it.
    thread = await _run_fix_tags("deleted-job", eligible_ids={"some-other-job"})

    assert _final_archive_state(thread) is not True


async def test_a_closed_job_missing_from_mongo_is_left_archived():
    thread = await _run_fix_tags(
        "deleted-job",
        eligible_ids={"some-other-job"},
        close_date=datetime.now(tz=timezone.utc) - timedelta(days=1),
    )

    # None means the thread was already archived and correctly tagged, so the
    # command spent no edit on it. What must not happen is an unarchive.
    assert _final_archive_state(thread) is not False
